...
def baca_menu():
    try:
        ...
        print(f"File '{MENU_FILE}' tidak ditemukan.")
        return {}
...
for nama_menu, harga in menu_data.items():
    # Tambahkan logika counter sesuai struktur eksisting
    ax_counter = plt.axes([0.01, 0.42, 0.1, 0.05])  # Sesuaikan posisi
    ax_counter.text(0.5, 0.5, f"{0} x Rp {harga:,}", fontsize=10, ha='center', va='center')
    ax_counter.axis('off')


def buat_tombol_menu():
    for idx, (nama_menu, harga) in enumerate(menu_data.items()):
        posisi_x = 0.1 * (idx % 5)  # Kolom ke-n
        posisi_y = 0.2 - (0.1 * (idx // 5))  # Baris ke-n (ditempatkan di bawah)

        # Membuat tombol dengan nama menu dan harga
        ax_button = plt.axes([posisi_x, posisi_y, 0.18, 0.05])  # Ukuran tombol
        button_label = f"{nama_menu}\nRp {harga:,}"
        button = Button(ax_button, button_label)
        
        # Klik tombol memunculkan pesan
        button.on_clicked(lambda event, menu=nama_menu: tampilkan_pesan(f"Menu '{menu}' dipilih!", color="blue"))
